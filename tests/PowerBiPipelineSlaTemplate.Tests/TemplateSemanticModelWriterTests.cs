...
{
    [Fact]
    public void PatchDataFolderBytes_ShouldPreserveSingleBackslashesAndCrLf()
    {
        const string original =
            "expression 'Other' = \r\n\t\tvalue = 1\r\n\r\n" +
            "expression DataFolder = \"data\" meta [IsParameterQuery=true, Type=\"Text\"]\r\n";
        const string dataFolder = "C:\\agent\\_work\\1\\s\\data";
        ...
        patched.Should().Contain($"expression DataFolder = \"{dataFolder}\" meta");
        patched.Should().NotContain("C:\\\\agent\\\\_work");
        patched.Should().Contain("\r\n");
        var expected = original.Replace("expression DataFolder = \"data\"", $"expression DataFolder = \"{dataFolder}\"");
        patched.Should().Be(expected);
    }

    [Fact]
    public void PatchDataFolderBytes_ShouldPreserveUtf8PreambleAndPatchExactBytes()
    {
        const string dataFolder = "C:\\agent\\data";
        var prefix = new UTF8Encoding(encoderShouldEmitUTF8Identifier: true).GetPreamble();
        var body = Encoding.UTF8.GetBytes("expression DataFolder = \"data\" meta [IsParameterQuery=true, Type=\"Text\"]\r\n");
        ...
        Encoding.UTF8.GetString(patchedBytes, prefix.Length, patchedBytes.Length - prefix.Length)
            .Should().Contain($"expression DataFolder = \"{dataFolder}\" meta");
    }
}

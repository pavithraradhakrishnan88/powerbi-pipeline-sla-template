...
function Get-CanonicalMeasureNames {
    ...
    if ($files.Count -ne 1) {
        throw ("VISUAL-PRESERVATION-GATE failed: expected exactly one canonical '_Measure Table.tmdl'; found {0}." -f $files.Count)
    }
    $names = New-Object 'System.Collections.Generic.HashSet[string]' ([StringComparer]::OrdinalIgnoreCase)
    foreach ($line in [IO.File]::ReadAllLines($files[0].FullName)) {
        if ($line -match "^\s*measure\s+'((?:''|[^'])+)'\s*=") {
            [void]$names.Add(($matches[1] -replace "''", "'"))
        }
    }
...
function Test-AllowedMeasureTransformation {
    ...
    if ($templateMatch.Success -and $generatedMatch.Success) {
        return $templateMatch.Groups['name'].Value -eq $generatedMatch.Groups['name'].Value -and
               $MeasureNames.Contains($generatedMatch.Groups['name'].Value)
    }
    return $false
}
...
function Convert-JsonNode {
    ...
    if ($Node -is [System.Management.Automation.PSCustomObject]) {
        ...
        foreach ($property in ($Node.PSObject.Properties | Sort-Object Name)) {
            $result[$property.Name] = Convert-JsonNode $property.Value
        }
        return $result
    }
    if ($Node -is [System.Collections.IDictionary]) {
        ...
        foreach ($key in ($Node.Keys | ForEach-Object {[string]$_} | Sort-Object)) {
            $result[$key] = Convert-JsonNode $Node[$key]
        }
        return $result
    }
...
function Compare-JsonNode {
    ...
    if ($TemplateNode -is [string] -and $GeneratedNode -is [string]) {
        if ($TemplateNode -ne $GeneratedNode -and !(Test-AllowedMeasureTransformation $TemplateNode $GeneratedNode $MeasureNames)) {
    ...
    if ($td -or $gd) {
        ...
        foreach ($key in $tk) {
            if ($key -in $gk) {
                foreach ($d in (Compare-JsonNode $TemplateNode[$key] $GeneratedNode[$key] ("{0}.{1}" -f $Path,$key) $MeasureNames)) { $differences.Add($d) }
            }
        }
        return $differences
    }
    ...
    if ($ta -or $ga) {
        ...
        if ($TemplateNode.Count -ne $GeneratedNode.Count) {
            $differences.Add(("{0}: array length differs (template={1}; generated={2})" -f $Path,$TemplateNode.Count,$GeneratedNode.Count)); return $differences
        }
        for ($i=0; $i -lt $TemplateNode.Count; $i++) {
            foreach ($d in (Compare-JsonNode $TemplateNode[$i] $GeneratedNode[$i] ("{0}[{1}]" -f $Path,$i) $MeasureNames)) { $differences.Add($d) }
        }
        return $differences
    }

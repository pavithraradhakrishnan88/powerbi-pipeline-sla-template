...
namespace PowerBiPipelineSlaTemplate.Core.Pbip
{
    public sealed class PbirReportWriter
    {
        ...
        public void WriteFromTemplate(string templateReportRootPath, string reportRootPath, string semanticModelRelativePath, string? semanticModelRootPath = null)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(semanticModelRootPath))
                    PbirMeasureReferenceMigrator.MigrateAndValidate(staging, semanticModelRootPath);
        ...
        private static void RegisterStaticResourceImages(string reportRoot)
        {
            ...
            var packages = report["resourcePackages"] as JsonArray ?? new JsonArray();
            report["resourcePackages"] = packages;
            JsonObject? registeredPackage = packages.OfType<JsonObject>().FirstOrDefault(p => string.Equals(p["name"]?.GetValue<string>(), "RegisteredResources", StringComparison.OrdinalIgnoreCase) && string.Equals(p["type"]?.GetValue<string>(), "RegisteredResources", StringComparison.OrdinalIgnoreCase));
        ...
        {
            var legacy = Path.Combine(reportRoot, "report.json"); if (File.Exists(legacy)) return legacy;
            var pbir = Path.Combine(reportRoot, "definition", "report.json"); if (File.Exists(pbir)) return pbir;
        ...
        {
            foreach (var candidate in new[] { Path.Combine(reportRoot, "StaticResources", "RegisteredResources"), Path.Combine(reportRoot, "staticResources", "RegisteredResources") }) if (Directory.Exists(candidate)) return candidate;

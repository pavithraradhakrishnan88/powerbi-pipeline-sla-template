...
namespace PowerBiPipelineSlaTemplate.Core.Pbip
{
    public sealed class PbirReportWriter
    {
        ...
        private static string BuildReportExtensionsJson() => JsonSerializer.Serialize(new Dictionary<string, object?> { ["$schema"] = "https://developer.microsoft.com/json-schemas/fabric/item/report/definition/reportExtension/1.0.0/schema.json", ["name"] = "extension", ["entities"] = Array.Empty<object>() }, JsonOptions);
        ...
        private static void ValidateGeneratedOutput(string root, ReportDefinitionDocument d) { foreach (var f in new[] { Path.Combine(root, "definition.pbir"), Path.Combine(root, "report.json"), Path.Combine(root, "reportExtensions.json") }) { if (!File.Exists(f)) throw new InvalidOperationException($"Generated PBIP output is missing required file '{f}'."); ValidateJsonFile(f); } foreach (var p in d.Pages) { var f = Path.Combine(root, "pages", SanitizeDirectoryName(ResolvePageId(p)), "page.json"); if (!File.Exists(f)) throw new InvalidOperationException($"Generated PBIP output is missing required page file '{f}'."); ValidateJsonFile(f); } }

...
internal static class PbirMeasureReferenceMigrator
{
    ...
    // Explicit compatibility map for measure names that have changed between the
    // authoritative report template and the generated inline-measure model.
    // Identity entries are intentionally included so the migration contract is
    // explicit and reviewable rather than relying on implicit name matching.
    private static readonly IReadOnlyDictionary<string, string> MeasureNameMap =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["Breached Count"] = "Breached Count",
            ["SLA Compliance %"] = "SLA Compliance %",
            ["Timeline Base"] = "Timeline Base",
            ["Floating Bar Duration"] = "Floating Bar Duration",
            ["Floating Bar Status"] = "Floating Bar Status",
            ["SLA Breach Color"] = "SLA Breach Color",
            ["Average Start Delay"] = "Average Start Delay",
            ["Average End Delay"] = "Average End Delay",
            ["Success Rate KPI"] = "Success Rate KPI",
    ...
    public static void MigrateAndValidate(string reportRoot, string semanticModelRoot)
    {
        ...
        ArgumentException.ThrowIfNullOrWhiteSpace(semanticModelRoot);

        var measures = ReadGeneratedMeasures(semanticModelRoot);
        if (measures.Count == 0)
            throw new InvalidOperationException($"No generated measures were found under '{semanticModelRoot}'.");

        var visualRoot = ResolveVisualRoot(reportRoot);
        var migrated = 0;
        var visualCount = 0;
        ...
        foreach (var visualPath in Directory.GetFiles(visualRoot, "visual.json", SearchOption.AllDirectories).OrderBy(x => x, StringComparer.OrdinalIgnoreCase))
        {
            ...
            {
                errors.Add($"{visualPath}: invalid JSON: {ex.Message}");
                continue;
            }
            ...
            if (changed)
            {
                File.WriteAllText(visualPath, root.ToJsonString(new JsonSerializerOptions { WriteIndented = true }), new System.Text.UTF8Encoding(false));
                migrated++;
            }
        }

        if (errors.Count > 0)
            throw new InvalidOperationException("PBIR measure-reference validation failed:" + Environment.NewLine + string.Join(Environment.NewLine, errors.Select(x => " - " + x)));
    ...
    private static bool MigrateNode(JsonNode node, string visualPath, List<string> errors, HashSet<(string Table, string Measure)> measures)
    {
        ...
        if (node is JsonObject obj)
        {
            if (obj["Measure"] is JsonObject measure && measure["Expression"] is JsonObject expression && expression["SourceRef"] is JsonObject sourceRef)
            {
                ...
                if (!string.IsNullOrWhiteSpace(entity) && !string.IsNullOrWhiteSpace(property))
                {
                    ...
                    {
                        ValidateMeasureReference(visualPath, entity!, mappedProperty, measures, errors);
                    }
                    else
                    {
                        measure["Property"] = mappedProperty;
                        changed |= !string.Equals(property, mappedProperty, StringComparison.Ordinal);
                    }
                }
            }
            ...
            foreach (var property in obj.ToList())
            {
                ...
                {
                    var text = property.Value.GetValueKind() == JsonValueKind.String ? property.Value.GetValue<string>() : null;
                    if (!string.IsNullOrWhiteSpace(text) && text.StartsWith(LegacyMeasuresTable + ".", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        property.Value = $"{GeneratedMeasuresTable}.{ResolveMeasureName(oldName)}";
                        changed = true;
                        continue;
                    }
                }

                if (MigrateNode(property.Value, visualPath, errors, measures))
                    changed = true;
            }
        }
        else if (node is JsonArray array)
        {
            foreach (var item in array)
                if (item is not null && MigrateNode(item, visualPath, errors, measures)) changed = true;
        }
    ...
    {
        if (!measures.Contains((entity, property)))
            errors.Add($"{visualPath}: Measure reference '{entity}.{property}' does not resolve to a generated measure.");
    }
    ...
    private static HashSet<(string Table, string Measure)> ReadGeneratedMeasures(string semanticModelRoot)
    {
        ...
        foreach (var file in Directory.GetFiles(semanticModelRoot, "*.tmdl", SearchOption.AllDirectories).OrderBy(x => x, StringComparer.OrdinalIgnoreCase))
        {
            foreach (var line in File.ReadLines(file))
            {
                ...
                if (table.Success)
                {
                    currentTable = table.Groups[1].Value.Trim().Trim('\'').Replace("''", "'", StringComparison.Ordinal);
                    continue;
                }

                var measure = measurePattern.Match(line);
                if (measure.Success && !string.IsNullOrWhiteSpace(currentTable))
                    result.Add((currentTable!, measure.Groups[1].Value.Replace("''", "'", StringComparison.Ordinal)));
            }
        }
    ...
    {
        var definition = Path.Combine(reportRoot, "definition", "pages");
        if (Directory.Exists(definition)) return definition;
        var legacy = Path.Combine(reportRoot, "pages");

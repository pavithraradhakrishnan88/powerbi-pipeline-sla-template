...
namespace PowerBiPipelineSlaTemplate.Core.Pbip
{
    internal static class PbirVisualContainerNormalizer
    {
        ...
        {
            var json = File.ReadAllText(visualPath);
            ...
            // visualContainerObjects belongs to the visual-container root, not inside visual.
            if (visual["visualContainerObjects"] is JsonNode nestedContainerObjects)
            {
                visual.Remove("visualContainerObjects");

                if (root["visualContainerObjects"] is null)
                {
                    root["visualContainerObjects"] = nestedContainerObjects;
                }
            }

            if (root["visualContainerObjects"] is JsonObject containerObjects &&
                containerObjects["title"] is JsonNode title)
            {
                // PBIR expects each visual-container object property, including title,
                // to be an array of object definitions. Preserve an existing array.
                if (title is JsonObject titleObject)
                {
                    containerObjects["title"] = new JsonArray(titleObject.DeepClone());
                }
                else if (title is not JsonArray)
                {
                    throw new InvalidOperationException(
                        $"PBIR visual '{visualPath}' has an invalid visualContainerObjects.title value. Expected an object or array of objects.");
                }
            }
